...
<script>
	$(function() {
		...
		$('#insertFormBtn').click(function() {
			location.href = '/board/memberQna/memberQnaForm';
		});
		...
		$(".goDetail").click(function() {
			...
			$("#detailForm").attr({
				"method" : "get",
				"action" : "/board/memberQna/memberQnaDetail"
			});
			$("#detailForm").submit();
		});
		...
		$(".paginate_button a").click(function(e) {
			e.preventDefault();
			$("#memberQnaSearchForm").find("input[name='pageNum']").val($(this).attr("href"));
			goPage();
		})
	...
	function goPage() {
		...
		$("#memberQnaSearchForm").attr({
			"method" : "get",
			"action" : "/board/memberQna/memberQnaList"
		});
		$("#memberQnaSearchForm").submit();
	}
	
</script>
<section class="container py-3">
	<div id="memberQnaSearch">
		<form id="memberQnaSearchForm" name="memberQnaSearchForm" class="row gx-3 gy-2 align-items-center">
			...
			<div class="col-auto">
				<select id="search" name="search" class="form-select">
					...
					<option value="member_qna_title">제목</option>
	        	...
	        	<c:choose>
	        		<c:when test="${not empty memberQnaList}">
	        			<c:forEach var="memberQna" items="${memberQnaList}" varStatus="status">
							<tr data-num="${memberQna.qna_no}">
								<td>${memberQna.qna_no}</td>
								<td class="text-center">
									${memberQna.qna_secret =='Y' ? '<i class="fa-solid fa-lock"></i>' : '<i class="fa-solid fa-lock-open"></i>'}
								</td>
								<td style="color: ${memberQna.qna_reply_status == 'Y' ? 'green' : 'red'}">
									${memberQna.qna_reply_status =='Y' ? '답변 완료' : '답변 대기'}
								</td>
								<td class="goDetail">${memberQna.qna_title}</td>
								<td>
									${memberQna.member_nickname}<br/>(${fn:substring(memberQna.id, 0, 3)}${'*****'})
								</td>
								<td>${memberQna.qna_regdate}</td>
							</tr>
						</c:forEach>
	        		</c:when>
        			<c:otherwise>
