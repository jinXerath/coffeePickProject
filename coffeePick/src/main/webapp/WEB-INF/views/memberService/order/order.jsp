...
<script type="text/javascript">
    ...
    // 주문 완료 페이지로의 이동 함수
    function redirectToOrderDetailPage(merchantUid) {
        setTimeout(function() {
                                        ...
                                        .done(
                                                function(data) {
                                                    if (rsp.paid_amount == data.response.amount) {
                                                        ...
                                                        redirectToOrderDetailPage(rsp.merchant_uid);
                                                    } else {
                                                        var msg = '결제에 실패하였습니다.';
                                                        msg += '에러내용 : '
                                                                + rsp.error_msg;
    ...
    function pickmoenyPay() {
        ...
        $.ajax({
                    ...
                    data : JSON
                            .stringify({
                                merchant_uid : "order_" + makeMerchantUid,
                                basicPrice : parseInt($("#totalPrice").text()
                                        .replace(/,/g, '')),
                                usePoint : parseInt($("#Point").val().replace(
                                        /,/g, '')),
                                request : $("#requestText").val(),
                                method : $("#paymentMethodText").val(),
                                storeName : $("td:eq(0)",
                                        "tr[data-cart-detail-no]:first").text(),
                                storePhone : $("td:eq(1)",
                                        "tr[data-cart-detail-no]:first").text(),
                                storeAddr : $("td:eq(2)",
                                        "tr[data-cart-detail-no]:first").text(),
                    ...
                    success : function(data) {
                        ...
                        redirectToOrderDetailPage("order_" + makeMerchantUid);
    ...
    $(function() {
        ...
        var tdElements = document
                .querySelectorAll('.cartDetailStoreId, .cartDetailStoreName, .cartDetailStorePhone, .cartDetailStoreAddr');
                ...
                .click(
                        function() {
                            ...
                            if (paymentMethodText === "0") {
                                ...
                                const usePickmoney = parseFloat($(
                                        "#usePickmoney").val()
                                        .replace(/,/g, ''));
                                const pickmoneyTotal = parseFloat("${pickmoneyInfo.pickmoney_total}"
                                        .replace(/,/g, ''));
                                ...
                                if (!$("#totalPrice").text() == $("#usePoint")
                                        .val()) {
                                    MsgBox
                                            .Alert("포인트결제는 결제가격만큼 포인트를 사용하셔야 가능합니다");
                                } else {
                                    pickmoenyPay();
                                }
                            }
        ...
        $("#point-table button.btn-primary")
                .click(
                        function() {
                            ...
                            let earnedPoint = parseFloat($("#earnedPoint")
                                    .val().replace(/,/g, ''));
