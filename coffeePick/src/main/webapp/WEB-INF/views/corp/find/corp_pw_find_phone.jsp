...
<meta charset="UTF-8">
		...
		$(function(){			
	    	...
	    	$("#next").click(function(){
	            ...
	            if(!chkData("#corp_name","이름을"))return;
	            if(!chkData("#corp_phone","핸드폰 번호를"))return;
	            else{          
	            $("#form").attr({
	                "method":"post",
	                "action":"/corp/corpPwFind_phone"
	            });
	            $("#form").submit();	    		
	    		}
	    	});
...
<label>id</label>	<input type="text" id="corp_id" name="corp_id" placeholder="ID" value="${param.corp_id}"/> <br/> 		
<label>이름</label>  	<input type="text" id="corp_name" name="corp_name" placeholder="성함" value="${param.corp_name}"/><br/>
<label>핸드폰 번호</label>	<input type="text" id="corp_phone" name="corp_phone" placeholder="핸드폰 번호" value="${param.corp_phone}"/>
