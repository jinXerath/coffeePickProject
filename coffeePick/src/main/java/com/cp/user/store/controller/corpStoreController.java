...
public class corpStoreController {
	...
	public String storeInfoRead(Model model, HttpSession session) {
		...
		CorpVO corpVO = (CorpVO)session.getAttribute("corp");
	      String corp_id = corpVO.getCorp_id();
...
//	}
	...
	public String storeInfoUpdate(@ModelAttribute StoreVO svo, Model model, HttpSession session) throws Exception {
		log.info("update 메소드 출력");
//		CorpVO corpVO = (CorpVO)session.getAttribute("corp");
		StoreVO storeVO = new StoreVO();
//		storeVO.setCorp_id(corpVO.getCorp_id());
		storeVO.setStore_id(svo.getStore_id());
		int result = 0;
		String url = "";
