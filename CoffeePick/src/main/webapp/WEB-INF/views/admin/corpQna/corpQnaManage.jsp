 ...
 * 문의 검색 및 페이징
 */
function getCorpQnaList() {
	$.ajax({
		type : "post",
		url : "/admin/corpQna/corpQnaList",
		data : {
		...
		success : function(data) {
			let corpQnaList = data.corpQnaList;
			...
			tbody.empty()
			if (corpQnaList.length > 0) {
				$.each(corpQnaList, function(index, corpQna){
					let row = $('<tr>').attr('data-no', corpQna.qna_no); 
					row.append('<td>' + corpQna.qna_no + '</td>');
	                row.append('<td>' + (corpQna.qna_secret=='Y' ?'<i class="fa-solid fa-lock"></i>' : '<i class="fa-solid fa-lock-open"></i>')+ '</td>');
	                
	                let textColor = (corpQna.qna_reply_status === 'Y') ? 'green' : 'red';
	                row.append('<td  style="color: ' + textColor + '">' + (corpQna.qna_reply_status=='Y' ? '답변 완료' : '답변 대기') + '</td>');
	                
	                row.append('<td>' + corpQna.qna_title + '</td>');
	                row.append('<td>' + (corpQna.store_name == null ? '등록된 매장 없음' : corpQna.store_name) + '<br/>' + corpQna.id + '</td>');
	                row.append('<td>' + corpQna.qna_regdate + '</td>');
	                let td7 = $('<td>');
	                let button = $('<button>').attr('type', 'button').addClass('btn btn-secondary corpQnaDetailBtn').text('조회');
	                td7.append(button);
	                row.append(td7);
	                tbody.append(row);
				});
 ...
 */
function getCorpQnaDetail(qna_no) {
	$.ajax({
		type : "post",
		url : "/admin/corpQna/corpQnaDetail",
		data : {
			"qna_no" : qna_no,
		},
		dataType : "json",
		success : function(data) {
	        ...
	        let qnaTable = '<table class="table table-bordered">' +
	            ...
	            '<td>' + (data.store_name == null ? '등록된 매장 없음' : data.store_name)+ '&nbsp;(' + data.id + ')</td>' +
	        ...
	        if (data.qna_img) {
	            qnaTable += '<tr>' +
	                ...
	                '<td colspan="5">' +
	                '<img src="/coffeePickStorage/corpQna/' + data.qna_img + '" width="300px"/>' +
	                '</td>' +
	                '</tr>';
	        }
	        qnaTable += '</table>';
	        ...
	        let qnaBtn = '<div id="qnaBtn" class="text-center">';
	        	...
	        	$.ajax({
	        		type : "post",
	        		url : "/board/corpQna/corpQnaReplyDetail",
	        		data : {
	        			"qna_no" : qna_no,
	        		},
	        		dataType : "json",
	        		success : function(replyData) {
						...
						if(replyData.qna_reply_img){
							...
							qnaReplyDiv += '<td>';
							qnaReplyDiv += '<img src="/coffeePickStorage/corpQnaReply/'+replyData.qna_reply_img+'" width="300px"/>';
							qnaReplyDiv += '</td>';
							qnaReplyDiv += '</tr>';
						}
						qnaReplyDiv += '</table>';
		...
		error : function() {
			alert("오류가 발생했습니다 getCorpQnaDetail.")
		}
	})
}
/**
 ...
 * 문의 답글을 삭제 할 수 있음
 */
function getCorpQnaReplyDelete(qna_no, qna_reply_img) {
	$.ajax({
		type : "post",
 ...
 * 문의 답변이 있으면 getCorpQnaReplyDelete를 사용하고 사용해야 함
 * 문의 글을 먼저 삭제하면 db에서는 문의 답변까지 삭제 하지만
 * 서버 사진 저장소에는 사진이 지워지지 않은 상태로 지워짐
 */
function getCorpQnaDelete(qna_no, qna_img) {
	$.ajax({
		type : "post",
		url : "/admin/corpQna/corpQnaDelete",
		data : {
			"qna_no" : qna_no,
			"qna_img" : qna_img
		},
		...
		error : function() {
			alert("오류가 발생했습니다.getCorpQnaDelete")
		}
	})
}
...
$(function(){
	...
	// 문의 상세보기
	$(document).on("click", ".corpQnaDetailBtn", function() {
		let qna_no = $(this).parents("tr").attr("data-no");
		getCorpQnaDetail(qna_no);
	})
	// 답변 없는 문의 글 삭제
	$(document).on("click", "#qnaDeleteBtn", function() {
		...
		if (confirm("상담/문의 글을 삭제하시겠습니까?")) {
			getCorpQnaDelete(qna_no, qna_img)
			getCorpQnaList();
		}
	})
	/*=====================================================================================*/
	...
	$(document).on("click", "#qnaReplyInsertBtn", function(event) {
		...
		$.ajax({
			type : "post",
			url : "/board/corpQna/corpQnaReplyInsert",
			data : formData,
			dataType : "text",
	...
	$(document).on("click", "#qnaReplyDeleteBtn", function() {
		...
		if (confirm("상담/문의 답변을 삭제하시겠습니까?")) {
			getCorpQnaReplyDelete(qna_no, qna_reply_img)
			alert("답변이 삭제되었습니다.")
			getCorpQnaDetail(qna_no);
			getCorpQnaList();
		}
	...
	$(document).on("click", "#qnaReplyUpdateFormBtn", function() {
		...
		if(qna_reply_img != "") {
			qnaReplyUpdateFormDiv += '<div class="mb-3" id="beforeImg">'+
									...
									'<figure class="figure">'+
									'<img src="/coffeePickStorage/corpQnaReply/'+qna_reply_img+'" class="figure-img img-fluid rounded" width="300px">'+
	...
	$(document).on("click", "#qnaReplyUpdateBtn", function(event) {
		...
		$.ajax({
			type : "post",
			url : "/board/corpQna/corpQnaReplyUpdate",
			data : formData,
			dataType : "text",
	        processData: false, // 중요: 데이터 처리를 jQuery에게 위임하지 않습니다.
	        contentType: false, 
...
<div class="container-fluid px-4">
	...
	<div class="card my-4">
	    ...
	    <div class="card-body">
				...
				<form id="adminSearchForm" name="adminSearchForm" class="row gx-3 gy-2 align-items-center">
					...
					<div class="col-auto">
						<select id="search" name="search" class="form-select">
							...
							<option value="corp_qna_title">문의 제목</option>
			...
			<!-- 상담/문의 리스트 -->
	       	<div id="corpQnaList">
	       		<table class="table table-striped table-hover align-middle" summary="매장회원 상담/문의 리스트">
